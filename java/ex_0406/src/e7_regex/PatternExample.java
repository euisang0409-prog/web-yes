package e7_regex;

import java.time.LocalDate;
import java.util.regex.Pattern;



public class PatternExample {
	public static void main(String[] args) {
		String regExp = "(02|010)-[0-9]{3,4}-[0-9]{4}";
		
		String data = "010-123-4567";
		
		boolean result = Pattern.matches(regExp, data);
		
		System.out.println(result);
		
		// ÁÖ¹Îµî·Ï¹øÈ£ Á¤±Ô½Ä ¸¸µé±â
		regExp = "[0-9]{6}-[0-9]{7}";
	      
	      String ssn = "001234-1234567";
	      
	      result = Pattern.matches(regExp, ssn);
	      
	      String birth = ssn.substring(0,6);
	      
	      int year = Integer.parseInt("20"+birth.substring(0,2));
	      System.out.println("»ıÀÏ : " + year);
	      int month = Integer.parseInt(birth.substring(2,4));
	      System.out.println("¿ù : " + month);
	      int day = Integer.parseInt(birth.substring(4,6));
	      System.out.println("ÀÏ : " +  day);
	      
	      try {
	         LocalDate.of(year, month,day);
	      } catch (Exception e) {
	         System.out.println("Àß¸øµÈ ³¯Â¥");
	      }
		
		// ¼ıÀÚ¸¸ ÀÔ·ÂµÇ¾ú´ÂÁö °Ë»ç
	    // ¹®ÀÚ¿­ÀÌ ¼ıÀÚ·Î¸¸ ÀÌ·ç¾îÁ® ÀÖ´ÂÁö °Ë»çÇÏ´Â ÄÚµå
	    // "12345"´Â Åë°ú
	    // "12a45"´Â ½ÇÆĞ
		
	      String str = "1234567890";
	      regExp = "[0-9]+";
		
		System.out.println(str.matches(regExp));
		
		for(int i = 0; i < str.length(); i++) {
			char ch = str.charAt(i);
			if(ch >= '1' && ch <= '9') {
				System.out.println("´Ù¸¥ ±ÛÀÚ°¡ Æ÷ÇÔµÊ");
				break;
			}
		}
		
		
		// ¾ÆÀÌµğ Çü½Ä°Ë»ç
		// ¿µ¾î ¼Ò¹®ÀÚ·Î ½ÃÀÛ
		// ¿µ¾î ¼Ò¹®ÀÚ + ¼ıÀÚ¸¸ Çã¿ë
		// 5 ~ 12ÀÚ
		
		str = "user01";
		regExp = "[a-z][a-z0-9]{4,11}";
		
		System.out.println(str.matches(regExp));
		
		// ÀÌ¸ŞÀÏ Çü½Ä °Ë»ç
		// ¾ÆÀÌµğºÎºĞ : ¿µ¹®,¼ıÀÚ,_
		// µµ¸ŞÀÎ Æ÷ÇÔ
		// ¿¹½Ã : test01@gmail.com
		
		str = "euisang0409@gmail.com";
		regExp = "[a-zA-Z0-9_]+@[a-zA-Z]+.[a-zA-Z]{2,}";
	System.out.println(str.matches(regExp));	
		
	String[] arr = {"123","abc","4567","a1b2"};
	
	regExp = "[0-9]+";
	
	for(String s : arr) {
		if(s.matches(regExp)) {
			System.out.println(s);
		}
	}
	
	String[] arr2 = {"È«±æµ¿","Tom","±èÃ¶¼ö","a1","±èÀÌ¹ÚÁ¶±æµ¿"};
	
	regExp = "[°¡-ÆR]{2,5}";
	
	for(String s : arr2) {
		if(s.matches(regExp)) {
			System.out.println(s);
		}
	}
	
	// ÇÑ±Û¸¸, 2~5±ÛÀÚ
	
	
	
	
	
	
	}
}
